...
(define tests
  (test-suite
   ...
   (check-equal? (vector-assoc 4 (vector 2 (cons 3 1) (cons 4 1) 7)) (cons 4 1) "vector-assoc test mixed")
   
   ; cached-assoc tests
   (check-equal? ((cached-assoc (list (cons 1 2) (cons 3 4)) 3) 3) (cons 3 4) "cached-assoc test")
   (check-equal? ((cached-assoc (list (cons 1 2) (cons 3 4)) 3) 5) #f "cached-assoc test no match")
